...
$exam=new ExamCollection(Exam::select('*')
                              ->whereRaw('FIND_IN_SET('.$camp[0]->state_id.',state_id)')->get());
...
$subject=Subject::all();
	$modedata=Mode::select('test_mode_id','test_mode_name',
		 ...
		 if ($request->hasFile('files')) 
	      {
	        ...
	        $path='/home/poojit24/public_html/athiratechnologies.com/xamp/sri_chaitanya/College/3_view_created_exam/uploads'.'/'.$request->Exam_Id;
	    	...
	    	$isupload=Exam::where('sl',$request->Exam_Id)
          		->update(['is_college_id_mobile_uploaded' => DB::raw("CONCAT(coalesce(is_college_id_mobile_uploaded,''),',','".$CAMPUS_NAME[0]['CAMPUS_ID']."')")
	            ...
	            return [
	            	'Login' => [
							'response_message'=>"success",
							'response_code'=>"1",
							'Image_Uploaded'=> '/home/poojit24/public_html/athiratechnologies.com/xamp/sri_chaitanya/College/3_view_created_exam/uploads'.'/'.$input,
							'size'=>$size
